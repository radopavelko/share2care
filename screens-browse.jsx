    ...
    <div style={{ display: 'flex', alignItems: 'center' }}>
      ...
      {extra > 0 && (
        <div style={{
          ...
          fontFamily: 'Inter, sans-serif', fontSize: size * 0.34, fontWeight: 700, color: T.inkSoft,
        }}>+{extra}</div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ padding: '14px 20px 0' }}>
      <window.Card style={{ padding: '22px 20px' }}>
        <div style={{ fontFamily: 'Archivo Black, sans-serif', fontWeight: 700, fontSize: 21, color: T.ink, letterSpacing: -0.3 }}>Get started</div>
        <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, color: T.inkSoft, marginTop: 4, marginBottom: 18 }}>Three steps and you’re sharing.</div>
          ...
          {steps.map((s, i) => (
            <div key={i} style={{ display: 'flex', gap: 13, alignItems: 'flex-start' }}>
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ fontFamily: 'Inter, sans-serif', fontWeight: 600, fontSize: 14.5, color: T.ink }}>{s.title}</div>
                <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, color: T.inkSoft, marginTop: 1, lineHeight: 1.45, textWrap: 'pretty' }}>{s.sub}</div>
              </div>
            </div>
          ))}
        </div>
    ...
    <div style={{ paddingBottom: 120 }}>
      <div style={{ padding: '46px 20px 6px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          <div style={{ minWidth: 0 }}>
            <span style={{ fontFamily: 'Archivo Black, sans-serif', fontSize: 22, color: T.ink, letterSpacing: -0.5, lineHeight: 1.05, textTransform: 'uppercase' }}>{app.group ? app.group.name : 'The shelf'}</span>
            <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 13.5, color: T.inkSoft, marginTop: 3 }}>{stackIds.length} {stackIds.length === 1 ? 'member' : 'members'} · {list.length} {list.length === 1 ? 'thing' : 'things'}</div>
          </div>
      ...
      <div style={{ padding: '12px 20px 8px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: T.surface, border: `1px solid ${T.line}`, borderRadius: 14, padding: '11px 14px', boxShadow: T.shadowSm }}>
          ...
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search the shelf…" style={{
            ...
            fontFamily: 'Inter, sans-serif', fontSize: 15.5, color: T.ink, minWidth: 0,
          }} />
      ...
      {incoming.length > 0 && (
        <div style={{ padding: '4px 20px 8px' }}>
          <div onClick={() => app.goTab('borrows')} style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: 'Inter, sans-serif', fontWeight: 600, fontSize: 14.5, color: T.accentDeep }}>{incoming.length} {incoming.length === 1 ? 'request' : 'requests'} for your things</div>
              <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, color: T.accentDeep, opacity: 0.8 }}>Tap to review in My Loans</div>
            </div>
      ...
      <div style={{ padding: '6px 20px 0', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        {list.map(it => (
          <window.Card key={it.id} onClick={() => app.openItem(it.id)} style={{ borderRadius: 18 }}>
            ...
            <div style={{ padding: '2px 11px 13px' }}>
              <div style={{ fontFamily: 'Inter, sans-serif', fontWeight: 600, fontSize: 15, color: T.ink, lineHeight: 1.2, marginBottom: 7, textWrap: 'pretty' }}>{it.name}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 9 }}>
                <window.Avatar user={it.ownerUid} size={18} />
                <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 12.5, color: T.inkSoft }}>{ownerLabel(it.ownerUid, uid)}</span>
              </div>
        ...
        {!q && list.length > 0 && (
          <button
            ...
            <span style={{ fontFamily: 'Inter, sans-serif', fontWeight: 600, fontSize: 13.5 }}>Add</span>
          </button>
        )}
      </div>
      {list.length === 0 && (
        q ? (
          <div style={{ textAlign: 'center', padding: '46px 30px', color: T.inkFaint, fontFamily: 'Inter, sans-serif', fontSize: 15 }}>
            Nothing matches “{q}”.
          </div>
        ) : app.group ? (
          <div style={{ textAlign: 'center', padding: '42px 30px', fontFamily: 'Inter, sans-serif' }}>
    ...
    <div style={{ position: 'absolute', inset: 0, background: T.bg, zIndex: 150, overflowY: 'auto', animation: 'screenIn .28s cubic-bezier(.16,1,.3,1) both' }}>
      ...
      <div style={{ padding: '18px 20px 140px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
          <div>
            <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 11.5, letterSpacing: 0.4, color: T.inkFaint, textTransform: 'lowercase' }}>{window.normCat(item.cat)}{item.cond && item.cond !== 'Good' ? ` · ${item.cond}` : ''}</div>
            <h1 style={{ fontFamily: 'Archivo Black, sans-serif', fontSize: 21, color: T.ink, letterSpacing: -0.5, margin: '5px 0 0', lineHeight: 1.15, textTransform: 'uppercase' }}>{item.name}</h1>
          </div>
        ...
        {market && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 12 }}>
            ...
            <span style={{ fontFamily: 'Archivo Black, sans-serif', fontWeight: 700, fontSize: 21, color: market.color, letterSpacing: -0.3 }}>{market.label}</span>
          </div>
        )}

        <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 15.5, lineHeight: 1.55, color: T.inkSoft, margin: '16px 0 22px', textWrap: 'pretty' }}>{item.desc}</p>
        ...
        {owner && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 13, background: T.surface, border: `1px solid ${T.lineSoft}`, borderRadius: 18, padding: 14, boxShadow: T.shadowSm }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: 'Inter, sans-serif', fontWeight: 600, fontSize: 15.5, color: T.ink }}>{isMine ? 'You' : owner.full}</div>
              {!isMine && owner.email && (
                <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, color: T.inkSoft, marginTop: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{owner.email}</div>
              )}
            </div>
          </div>
        )}
        ...
        {isMine && app.groups.length > 0 && (
          <div style={{ marginTop: 18 }}>
            <div style={{ fontFamily: 'Inter, sans-serif', fontWeight: 600, fontSize: 13.5, color: T.ink, marginBottom: 9 }}>Shared with</div>
              ...
              {app.groups.map(g => {
                ...
                return (
                  <button key={g.id} onClick={() => app.toggleItemGroup(item, g.id)} style={{
                    ...
                    cursor: 'pointer', fontFamily: 'Inter, sans-serif', fontSize: 13.5, fontWeight: 600,
            ...
            {(!item.groups || item.groups.length === 0) && (
              <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 12.5, color: T.inkFaint, marginTop: 8 }}>Not in a group yet — only you can see it.</div>
            )}
          </div>
        )}

        {item.status === 'out' && borrower && (
          <div style={{ marginTop: 14, display: 'flex', alignItems: 'center', gap: 10, fontFamily: 'Inter, sans-serif', fontSize: 14, color: T.inkSoft, padding: '0 4px' }}>
        ...
        {myPending && (
          <div style={{ marginTop: 14, display: 'flex', alignItems: 'center', gap: 10, fontFamily: 'Inter, sans-serif', fontSize: 14, color: T.accentDeep, padding: '0 4px' }}>
  ...
  return (
    <window.Sheet open={open} onClose={onClose} title={`Ask ${owner.name} to borrow`}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        ...
        <div>
          <div style={{ fontFamily: 'Inter, sans-serif', fontWeight: 600, fontSize: 15.5, color: T.ink }}>{item.name}</div>
          <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, color: T.inkSoft }}>{window.normCat(item.cat)}</div>
        </div>
      </div>

      <div style={{ fontFamily: 'Inter, sans-serif', fontWeight: 600, fontSize: 14, color: T.ink, marginBottom: 9 }}>How long do you need it?</div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9, marginBottom: 20 }}>
        {options.map((o, i) => (
          <button key={i} onClick={() => setSel(i)} style={{
            ...
            <div style={{ fontFamily: 'Inter, sans-serif', fontWeight: 600, fontSize: 14.5, color: sel === i ? T.accentDeep : T.ink }}>{o.label}</div>
            <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 12.5, color: T.inkSoft, marginTop: 2 }}>back by {window.fmtDate(window.isoFromOffset(o.days))}</div>
          </button>
        ))}
      </div>

      <div style={{ fontFamily: 'Inter, sans-serif', fontWeight: 600, fontSize: 14, color: T.ink, marginBottom: 9 }}>Add a note <span style={{ color: T.inkFaint, fontWeight: 400 }}>(optional)</span></div>
      <textarea value={note} onChange={e => setNote(e.target.value)} placeholder={`Hi ${owner.name}! Mind if I borrow this?`} rows={2} style={{
        ...
        borderRadius: 14, padding: '12px 14px', fontFamily: 'Inter, sans-serif', fontSize: 14.5,
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 2 }}>
            ...
            <span style={{ fontFamily: 'Inter, sans-serif', fontWeight: 700, fontSize: 14, color: market.color }}>{market.label}</span>
          </div>
        </div>
      </div>

      <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, color: T.inkSoft, marginBottom: 16, lineHeight: 1.5, textWrap: 'pretty' }}>
        {market.kind === 'sell'
...
function Field({ label, children }) {
  ...
  return (
    <div style={{ marginBottom: 18 }}>
      <div style={{ fontFamily: 'Inter, sans-serif', fontWeight: 600, fontSize: 13.5, color: T.ink, marginBottom: 8 }}>{label}</div>
      {children}
    </div>
  );
}
function inputStyle(T) {
  return {
    ...
    borderRadius: 13, padding: '12px 14px', fontFamily: 'Inter, sans-serif',
