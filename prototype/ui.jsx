// ui.jsx — design tokens + shared presentational components for Share2

const THEME = {
  bg:        '#F3EDE5', // warm sand page
  surface:   '#FFFFFF',
  surfaceAlt:'#FBF6EF',
  ink:       '#2C2722',
  inkSoft:   '#6F665C',
  ...
  line:      '#E8E0D5',
  lineSoft:  '#F0EAE1',
  accent:    '#C2693F', // clay / terracotta
  accentDeep:'#A8542F',
  accentSoft:'#F3E1D5',
  good:      '#6E8B66', // sage — available
  goodSoft:  '#E4EADD',
  warn:      '#B07A2E', // amber — due soon
  over:      '#B0503B', // overdue
...
function Avatar({ user, size = 40, ring = false }) {
  ...
  return (
    <div style={{
      ...
      boxShadow: ring ? `0 0 0 2.5px ${THEME.surface}, 0 0 0 4px ${u.color}33` : 'none',
      userSelect: 'none',
    }}>{u.name[0]}</div>
  );
}

// ── Category thumbnail (warm placeholder tile) ─────────────────
...
function Btn({ children, onClick, variant = 'primary', full = false, size = 'md', disabled = false, style = {} }) {
  ...
  const variants = {
    ...
    danger:  { background: '#FromError', color: THEME.over, border: `1.5px solid #EAD3CC`, },
    good:    { background: THEME.good, color: '#fff', border: 'none', boxShadow: '0 2px 10px rgba(110,139,102,0.3)' },
  };
  const v = variants[variant] || variants.primary;
  if (variant === 'danger') v.background = '#FBF1EE';
  return (
