// screens-browse.jsx — Browse (group shelf) + Item detail + borrow sheet + form helpers

const { useState: useStateB } = React;

function ownerLabel(id) {
  const u = window.MEMBERS[id];
  return u && u.you ? 'You' : (u ? u.name : '');
}
...
function BrowseScreen({ app }) {
  ...
  const incoming = app.requests.filter(r => r.dir === 'in' && r.status === 'pending' && r.group === g.id);

  const inGroup = app.items.filter(it => (it.groups || []).includes(g.id));
  const cats = ['All', ...window.CATEGORIES];
  let list = inGroup.filter(it => {
    const okCat = cat === 'All' || it.cat === cat;
    const okQ = !q || (it.name + ' ' + it.cat + ' ' + ownerLabel(it.owner)).toLowerCase().includes(q.toLowerCase());
    return okCat && okQ;
  });
  ...
  return (
    <div style={{ paddingBottom: 120 }}>
      ...
      <div style={{ padding: '52px 20px 6px' }}>
        <button onClick={() => app.openModal('switcher')} style={{
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
              <span style={{ fontFamily: 'Bricolage Grotesque, sans-serif', fontWeight: 700, fontSize: 28, color: T.ink, letterSpacing: -0.5, lineHeight: 1.05, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{g.name}</span>
              <span style={{ width: 24, height: 24, borderRadius: 8, background: T.accentSoft, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none"><path d="M6 9l6 6 6-6" stroke={T.accentDeep} strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" /></svg>
              </span>
            </div>
            <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 13.5, color: T.inkSoft, marginTop: 3 }}>{g.members.length} members · {inGroup.length} shared</div>
          </div>
          <AvatarStack ids={others} />
        </button>
      </div>
      ...
      <div style={{ padding: '12px 20px 8px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: T.surface, border: `1px solid ${T.line}`, borderRadius: 14, padding: '11px 14px', boxShadow: T.shadowSm }}>
          ...
          <input value={q} onChange={e => setQ(e.target.value)} placeholder={`Search ${g.name}…`} style={{
          ...
          <window.Card key={it.id} onClick={() => app.openItem(it.id)} style={{ borderRadius: 18 }}>
            ...
            <div style={{ padding: '2px 11px 13px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 9 }}>
                <window.Avatar user={it.owner} size={18} />
                <span style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 12.5, color: T.inkSoft }}>{ownerLabel(it.owner)}</span>
              </div>
      ...
      {list.length === 0 && (
        <div style={{ textAlign: 'center', padding: '46px 30px', color: T.inkFaint, fontFamily: 'DM Sans, sans-serif' }}>
          <div style={{ fontSize: 15, marginBottom: 14 }}>{q ? `Nothing matches “${q}”.` : `Nothing on ${g.name}'s shelf yet.`}</div>
...
function ItemDetail({ app, item }) {
  ...
  const owner = window.MEMBERS[item.owner];
  const isMine = owner.you;
  const borrower = item.borrower ? window.MEMBERS[item.borrower] : null;
  const myPending = app.requests.find(r => r.itemId === item.id && r.from === 'u1' && r.status === 'pending');
      ...
      <div style={{ padding: '18px 20px 140px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 13, background: T.surface, border: `1px solid ${T.lineSoft}`, borderRadius: 18, padding: 14, boxShadow: T.shadowSm }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontFamily: 'DM Sans, sans-serif', fontSize: 13, color: T.inkSoft, marginTop: 1 }}>
              <window.Icon name="pin" size={13} color={T.inkFaint} /> {owner.unit} · {app.group.name}
            </div>
          </div>
        </div>
        ...
        {item.status === 'out' && borrower && (
          <div style={{ marginTop: 14, display: 'flex', alignItems: 'center', gap: 10, fontFamily: 'DM Sans, sans-serif', fontSize: 14, color: T.inkSoft, padding: '0 4px' }}>
            ...
            {borrower.you ? `You have this until ${window.fmtDate(item.due)}` : `With ${borrower.name} · back ${window.fmtDate(item.due)}`}
          </div>
        )}
        {myPending && (
          <div style={{ marginTop: 14, display: 'flex', alignItems: 'center', gap: 10, fontFamily: 'DM Sans, sans-serif', fontSize: 14, color: T.accentDeep, padding: '0 4px' }}>
            ...
            Request sent — waiting for {owner.name} to confirm
          </div>
        )}
      </div>

      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '14px 20px 30px', background: `linear-gradient(to top, ${T.bg} 72%, transparent)` }}>
        {isMine ? (
    ...
    <window.Sheet open={open} onClose={onClose} title={`Ask ${owner.name} to borrow`}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        ...
        <div>
          ...
          <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 13, color: T.inkSoft }}>{item.cat} · {owner.unit}</div>
        </div>
      </div>
